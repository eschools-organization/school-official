...
const EditClassForm: React.FC<EditClassFormProps> = ({ onUpdateClass, onCancel, classes, teachers, subjects: initialSubjects, onSubjectUpdated }) => {
    const [selectedClassId, setSelectedClassId] = useState<string>(() => {
        if (typeof window !== 'undefined') {
            ...
            const saved = localStorage.getItem('edit_class_selected_id');
    ...
    useEffect(() => {
        if (selectedClassId && typeof window !== 'undefined') {
            localStorage.setItem('edit_class_selected_id', selectedClassId);
    ...
    useEffect(() => {
        if (classes.length > 0) {
            const savedId = selectedClassId || (typeof window !== 'undefined' ? localStorage.getItem('edit_class_selected_id') : '');
            const targetClass = savedId
                ? (classes.find(c => c._id === savedId) || classes[0])
